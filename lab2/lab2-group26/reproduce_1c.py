...
def plot_figure_9c(distribution_dict, num_servers, num_switches, num_ports, iterarions):
    
    if len(distribution_dict) == 1:
        plt.bar(distribution_dict[0].keys(), distribution_dict[0].values())
    ...
    # else if argumetns are given, execute custom 
        ...
        try: 
            ...
            if not run_both:
                ...
                plot_figure_9c([total_pathlen_distr], num_servers, num_switches, num_ports, iterations)
            else:
